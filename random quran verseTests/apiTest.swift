//
//  apiTest.swift
//  random quran verseTests
//
//  Created by Iman Morshed on 11/8/23.
//
...
func fetchQuranVerse(completion: @escaping QuranVerseCompletion) {
    let headers = [
        "X-RapidAPI-Key": "05826406d6msh37c7156f2232f2cp1e109fjsn3e5b2d5afdf1",
        "X-RapidAPI-Host": "al-quran1.p.rapidapi.com"
    ]

    let request = NSMutableURLRequest(url: NSURL(string: "https://al-quran1.p.rapidapi.com/\(surahNumber)/\(ayahNumber)")! as URL,
...
for surahNumber in 1...114{
    for ayahNumber in 1...versesPerChapter[surahNumber]{
        //call the api then test it against another
        /*if incorrect {
            incorrectResponses.push([surahNumber,ayahNumber])}
         */
    }
}
/**
 al araf7:42 is wrong
 */
